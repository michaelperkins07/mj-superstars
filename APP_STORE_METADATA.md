# Top Performer - App Store Metadata Guide

**Last Updated:** February 11, 2026
**Target Platform:** iOS 14.0+
**App Store Connect ID:** 6743862814

---

## REQUIRED METADATA FIELDS

### 1. App Name
**Character Limit:** 30 characters
**Current:** Top Performer
**Status:** ✓ Valid (13 characters)

---

### 2. Subtitle
**Character Limit:** 30 characters
**Recommended:** AI + Emotional Intelligence Coach
**Status:** ✓ Valid (30 characters)

Other options:
- Your AI Wellness Coach (23 characters)
- AI Coaching, Real Connection (29 characters)

---

### 3. Description
**Character Limit:** 4000 characters
**Tone:** Friendly, warm, empowering, professional

```
Experience personalized coaching with Top Performer, where AI
meets Emotional Intelligence. Real people helping real people —
get support from coaches who understand your journey and adapt
to your unique communication style.

KEY FEATURES:

🧠 AI + EI Coaching
Get personalized guidance, encouragement, and support tailored
to your unique needs. Your coach learns your communication
preferences and adapts their approach to match your style.
Choose from multiple conversation modes: Perk Mode for daily
motivation, Empathy Mode for emotional support, Problem Solve
Mode for working through challenges, and Elite Communicator
Mode for leveling up your people skills.

📊 Mood Tracking & Insights
Log your daily emotions, track patterns, and gain meaningful
insights into your wellness journey. Understand what influences
your mood and build awareness.

🔥 Habit Building & Gamification
Transform wellness into a game. Build daily streaks, earn
rewards, and unlock achievements. Every action counts—consistency
beats perfection. Our "Everything is Reps" philosophy celebrates
small wins that compound into big changes.

💪 Supportive Community
Connect with others on their wellness journey. Share experiences,
find inspiration, and grow together in a safe, moderated community.

📓 Digital Journaling
Reflect on your day, process your emotions, and create a personal
wellness record. Your thoughts, your space, always private.

⚙️ Personalized Experience
Customize check-in times, communication style, and wellness goals.
Top Performer adapts to YOU, not the other way around.

🎯 Goal Tracking & Vision Board
Set meaningful wellness goals and visualize your progress. Create
a vision board to inspire daily action.

MENTAL HEALTH SUPPORT:

Top Performer is NOT a replacement for professional mental
health care. If you're in crisis, please contact a mental health
professional or crisis hotline immediately. Our app complements
therapy, medication, and professional care.

Crisis Resources:
- National Suicide Prevention Lifeline: 988 (US)
- Crisis Text Line: Text HOME to 741741
- International Association for Suicide Prevention: https://www.iasp.info/resources/Crisis_Centres/

PRIVACY & SECURITY:

Your mental health data is sacred. We employ:
- End-to-end encryption for sensitive data
- HIPAA-compliant infrastructure
- Strict data privacy policies (never sold to third parties)
- Regular security audits
- Transparent data practices

SUBSCRIPTION:

Free tier includes basic mood tracking and community features.
Elite ($19.99/mo — first month free) unlocks unlimited AI coaching,
advanced insights, and exclusive community features. Bet on yourself
for the ultimate return.

Join thousands using Top Performer to build better mental
health habits. Your journey to wellness starts now.

---

Made with ❤️ by the Top Performer team.
```

**Character Count:** ~1,850 (well within 4000 limit)

---

### 4. Promotional Text
**Character Limit:** 170 characters
**Update Frequency:** Can change without App Review

**Current:**
```
New year, new habits! Get started with personalized AI coaching
that adapts to your style today.
```

**Alternative versions for seasonal updates:**
```
Join 10K+ users building better mental health habits daily.
```

```
Your AI coach is ready. Start your wellness journey today.
```

---

### 5. Keywords
**Character Limit:** 100 characters (comma-separated)
**Tip:** Focus on search volume, specificity, and relevance

**Recommended Keywords:**
```
mental health, wellness, meditation, mood tracker, anxiety, stress relief, mental health app, habit tracker, journaling, AI coach, self-care, emotional health, mindfulness, therapy, mental fitness
```

**Character Count:** 158 (need to trim to 100)

**Optimized (100 chars max):**
```
mental health, mood tracker, wellness, anxiety, meditation, habit tracker, AI coach, journaling, stress
```

---

### 6. Category
**Primary Category:** Health & Fitness
**Secondary Category:** Lifestyle

---

### 7. Age Rating / Content Rating Questionnaire

**Age Rating:** 4+ (Green)

**Content Assessment:**
- [ ] No content restricted to adults
- [ ] No gambling
- [ ] No alcohol, tobacco, or drugs
- [ ] No medical advice (we explicitly state this is NOT a medical app)
- [ ] No violence, horror, or profanity
- [ ] No dating/sexual content
- [ ] No user-generated content restrictions
- [ ] Personal data collection: Yes (email, mood data)

**Mental Health Disclaimer Required in App:**
```
IMPORTANT: Top Performer is a wellness companion tool, not a
medical app or replacement for professional mental health care.
If you're experiencing a mental health crisis, please contact:
- National Suicide Prevention Lifeline: 988 (US)
- Crisis Text Line: Text HOME to 741741
- Your local emergency services or mental health professional
```

---

### 8. Privacy Policy URL
**URL:** https://mj-superstars-app.onrender.com/privacy
**Status:** Must be published before App Review

**Privacy Policy Must Cover:**
- What personal data is collected (email, mood data, journal entries)
- How data is used (ML personalization, aggregate analytics)
- Data retention periods
- User rights (data access, deletion, export)
- Third-party data sharing (clearly: we don't sell data)
- Security measures
- GDPR/CCPA compliance
- Cookie usage
- Children's privacy (if applicable under 13)

---

### 9. Support URL
**URL:** https://mj-superstars-app.onrender.com/support
**Status:** Must be published before App Review

**Support Page Should Include:**
- FAQ section
- Contact form or email
- Common troubleshooting
- Feature documentation
- How to report bugs
- How to suggest features
- Refund policy for in-app purchases

---

### 10. What's New (Release Notes)
**For Version 1.0 Launch:**

```
Welcome to Top Performer!

We're thrilled to introduce your personal AI + Emotional
Intelligence coaching companion. This initial release includes:

FEATURES:
- Beautiful, guided onboarding experience
- AI coach with personalized conversation and 5 coaching modes
- Daily mood tracking with emoji expression
- Mood trend insights and pattern recognition
- Digital journaling with privacy protection
- Customizable check-in times and preferences
- Community discussion boards (beta)
- Habit tracking with streak rewards
- Achievement system for consistency
- Vision board for goal visualization
- Profile customization and preferences

IMPORTANT:
- Please review our Mental Health Disclaimer in Settings
- Not a replacement for professional mental health care
- For mental health crises, contact emergency services

FEEDBACK:
We love hearing from you! Use the Feedback button in Settings
to share your experience, report bugs, or suggest features.

Thank you for being part of the wellness revolution.
```

---

## SCREENSHOT REQUIREMENTS & DESCRIPTIONS

### Device Requirements
- **Primary:** iPhone 15 Pro Max (6.7" Super Retina XDR)
- **Secondary:** iPhone 11 Pro Max (6.5" Liquid Retina)
- **Resolution:** 1284 x 2778 pixels (6.7")
- **Format:** PNG or JPG
- **Maximum:** 5 MB per file

### Recommended Screenshots (5-8 screens)

**Screenshot 1: Welcome/Hero**
- Show: Onboarding screen with "Meet Your Coach" title
- Emoji: 🤖
- Text overlay (optional): "Your Personal AI + EI Coach"

**Screenshot 2: Mood Tracking**
- Show: Mood picker interface with emoji options
- Highlight: Simple, engaging mood selection
- Subtitle: "Track Your Emotional Journey Daily"

**Screenshot 3: AI Coach Interaction**
- Show: Chat interface with coach
- Highlight: Personalized conversation, warm tone
- Subtitle: "Get Support When You Need It"

**Screenshot 4: Habit Tracking**
- Show: Habit/streak interface with achievements
- Highlight: Gamification, rewards, fire emoji streak
- Subtitle: "Build Consistency, Celebrate Progress"

**Screenshot 5: Community**
- Show: Community discussion board with posts
- Highlight: Supportive comments, engagement
- Subtitle: "Connect With Others Like You"

**Screenshot 6: Insights Dashboard**
- Show: Mood trends, charts, weekly summary
- Highlight: Data visualization, progress tracking
- Subtitle: "Understand Your Patterns"

**Screenshot 7: Journaling**
- Show: Journal entry with rich text editing
- Highlight: Privacy, reflection space
- Subtitle: "Reflect and Process Privately"

**Screenshot 8: Personalization**
- Show: Settings with customization options
- Highlight: Communication style, coaching modes
- Subtitle: "Customize Your Experience"

---

## APP REVIEW INFORMATION

### Review Notes (Required for Initial Submission)

```
ABOUT TOP PERFORMER:

Top Performer is a wellness companion app that provides mental
health support through AI coaching, mood tracking, journaling,
and community features. Our primary user is anyone interested in
their mental health and wellness journey.

MENTAL HEALTH DISCLAIMER:
This app is NOT a medical device and NOT a replacement for
professional mental health care. We explicitly state this in our
onboarding and settings. Users are encouraged to work with
licensed mental health professionals for serious conditions.

KEY FEATURES:
1. AI Coach: Personalized conversations with 5 coaching modes
   (no medical advice given)
2. Mood Tracking: Simple emotion logging (not diagnostic)
3. Journaling: Private reflection space
4. Habit Tracking: Gamified wellness habit building
5. Community: Moderated discussion boards
6. Insights: Trend visualization and awareness tools

PRIVACY & SECURITY:
- Email authentication (standard)
- Encrypted data transmission (TLS 1.2+)
- Privacy policy: https://mj-superstars-app.onrender.com/privacy
- No third-party data sharing
- Users can delete their account and data anytime

TEST ACCOUNT CREDENTIALS:
Email: review@testaccount.com
Password: TestReview2026!
Full Name: Test Reviewer

NOTE: The test account is pre-populated with sample data and
pre-completed onboarding for faster review. The account has all
features unlocked for evaluation.

POTENTIAL CONCERNS & CLARIFICATIONS:
- No harmful content
- No medical claims (explicit disclaimers present)
- Age-appropriate (4+)
- No dark patterns or deceptive practices
- Transparent about app limitations
- Clear mental health resources for crisis situations

SUPPORT:
Support page: https://mj-superstars-app.onrender.com/support
Contact: support@topperformer.app

We're happy to answer any questions during review.
```

---

## TEST ACCOUNT INFORMATION

**For App Review:**
```
Email: review@testaccount.com
Password: TestReview2026!
Name: Test Reviewer
Phone: +1 (555) 123-4567
```

**Account Features:**
- All features unlocked (free + Elite)
- Pre-loaded with 30 days of sample mood data
- Sample journal entries
- Several achievements unlocked
- Active community posts

**Testing Suggestions for Reviewers:**
1. Complete onboarding (8 screens, ~3 minutes)
2. Log a mood and view mood history
3. View trending insights
4. Read and write a journal entry
5. View community discussion
6. Check all settings and customizations
7. Try "Skip Tutorial" from onboarding screen
8. Test account deletion from Settings

---

## CONTENT RIGHTS & DECLARATIONS

### Music & Audio
- [ ] App uses only royalty-free background music
- [ ] All music licensed from Freepik/Pixabay
- [ ] No copyrighted music

### Images
- [ ] All emoji are Apple's system emoji
- [ ] No third-party logos
- [ ] All app screenshots are original

### Text Content
- [ ] All marketing copy original
- [ ] Mental health statements reviewed by professionals
- [ ] No medical claims
- [ ] Proper attribution for any referenced research

### Third-Party Libraries
- [ ] React (MIT)
- [ ] Framer Motion (MIT)
- [ ] All dependencies comply with App Store policies

---

## PRICING & MONETIZATION

### Free Tier
- Basic mood tracking (3x daily limit)
- Community access (limited)
- Basic journaling
- Onboarding & tutorial

### Elite Subscription
- **Price:** $19.99/month or $199.99/year
- **Billing:** Via App Store In-App Purchase
- **Free Trial:** First month free (30-day trial)
- **Cancellation:** Easy cancel in Settings
- Features: Unlimited AI coaching, advanced insights, priority support

---

## RELEASE CHECKLIST

- [x] App name finalized (30 chars max)
- [x] Subtitle created (30 chars max)
- [x] Description written (4000 chars max)
- [x] Keywords optimized (100 chars max)
- [x] Category selected
- [ ] Privacy policy published online
- [ ] Support page published online
- [ ] 5-8 screenshots captured and optimized
- [ ] Age rating questionnaire completed
- [ ] Test account created and documented
- [ ] Review notes prepared
- [ ] Bundle ID verified: com.topperformer.app
- [ ] Version number set to 1.0
- [ ] Build number incremented
- [ ] All content rights reviewed
- [ ] Marketing copy proofread
- [ ] Mental health disclaimers verified

---

## QUICK REFERENCE COPY-PASTE

### For App Store Connect Form

**Name:** Top Performer

**Subtitle:** AI + Emotional Intelligence Coach

**Keywords:** mental health, mood tracker, wellness, anxiety, meditation, habit tracker, AI coach, journaling, stress

**Category:** Health & Fitness

**Description:** [Use the description from Section 3 above]

**Support URL:** https://mj-superstars-app.onrender.com/support

**Privacy Policy URL:** https://mj-superstars-app.onrender.com/privacy

**Contact Email:** support@topperformer.app

---

## MARKETING CAMPAIGN HASHTAGS

#TopPerformer #MentalWellness #AICoach #MentalHealthApp #WellnessJourney #MindfulnessMoment #MentalHealthMatters #WellnessHabits #AnxietySupport #EICoaching

---

**Document Status:** Ready for submission
**Last Reviewed:** February 11, 2026
**Next Review:** Before each version update
